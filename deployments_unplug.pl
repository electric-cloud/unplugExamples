...
#
# Use only with Electric Flow version 7.0 or later
#
# Description: Plot deployment history
#
# Run from the command line to debug:
#	ec-perl deployments_unplug.pl
#
# Load content into unplug location:
# 	ectool setProperty /server/unplug/v8 --valueFile deployments_unplug.pl
#
...
#   9/16/2016 - 1.1 Updated for CEV-11985 workaround
...
for my $project (@projects) {
	...
	$deploymentHistoryItems = $ec->getDeploymentHistoryItems({
			...
			})->{responses}[0]->{deploymentHistoryItem};
